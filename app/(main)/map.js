...
export default function MapScreen() {
  ...
  const points = useMemo(() => {
    if (isEvents) {
      ...
      return list.map((e) => ({ ...e, _kind: 'event' }));
    }
    const list = active ? SPOTS.filter((s) => s.category === active) : SPOTS;
    return list.map((s) => ({ ...s, _kind: 'spot' }));
  }, [mode, active]);
  ...
  return (
    <View style={styles.root}>
      <SpotMap spots={points} onSelect={open} />

      <View style={[styles.top, { paddingTop: insets.top + 8 }]} pointerEvents="box-none">
        <View style={styles.searchRow}>
          <View style={styles.search}>
            <Ionicons name="search" size={18} color={colors.textOnLightMuted} />
            <Text style={styles.searchTxt}>Search {isEvents ? 'events' : 'spots'} in Vizag</Text>
          </View>
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + 78 }]} pointerEvents="box-none">
        <FlatList
          ...
          renderItem={({ item }) =>
            item._kind === 'event'
              ? <EventCard event={item} onPress={() => open(item)} />
              : <SpotCard spot={item} onPress={() => open(item)} />
          }
        />
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  searchTxt: { fontFamily: fonts.medium, fontSize: 15, color: colors.textOnLightMuted },
  avatarBtn: {
    width: 48, height: 48, borderRadius: 24, backgroundColor: colors.maroon,
    alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#fff',
    shadowColor: '#000', shadowOpacity: 0.14, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 5,
  },
  avatarTxt: { color: '#fff', fontFamily: fonts.display, fontSize: 22 },
  ...
  card: {
    width: 240, marginRight: 12, backgroundColor: '#fff', borderRadius: radius.lg, overflow: 'hidden',
