...
import { clearRankings, currentUserId, fetchUserRankings, syncRankings } from './supabase';
...
export async function ensureLoaded() {
  ...
  loaded = true;
  // fresh device + signed-in account -> restore rankings from the cloud
  const empty = !buckets.liked.length && !buckets.fine.length && !buckets.disliked.length;
  if (empty) restoreFromCloud();
}
...
async function restoreFromCloud() {
  try {
    ...
    if (!uid) return;
    const rows = await fetchUserRankings(uid);
    if (!rows.length) return;
    const next = { liked: [], fine: [], disliked: [] };
    rows
      .sort((a, b) => (a.rank ?? 0) - (b.rank ?? 0))
      .forEach((r) => { if (next[r.sentiment]) next[r.sentiment].push(r.spot_id); });
    // only apply if the user still hasn't ranked anything locally
    if (!buckets.liked.length && !buckets.fine.length && !buckets.disliked.length) {
      buckets = next;
      await persist();
      notify();
    }
...
export async function placeSpot(id, sentiment, index) {
  ...
  touchStreak();
  const snap = snapshot();
  syncRankings(snap.ordered); // background push when signed in
