...
// Fire-and-forget: your look travels with your profile.
export function syncCharacter(config) {
  (async () => {
    const uid = await currentUserId();
    if (!uid) return;
    await supabase.from('profiles').update({ character: config }).eq('id', uid);
  })().catch(() => {});
}

// Characters for a set of users (map presence, profiles).
export async function fetchCharacters(userIds) {
  try {
    ...
    if (error || !data) return {};
    const out = {};
    for (const r of data) out[r.id] = { username: r.username, character: r.character };
    return out;
  } catch {
    return {};
  }
